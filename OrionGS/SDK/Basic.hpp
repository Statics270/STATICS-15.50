...
static void* (*FMemory_Realloc)(void* Memory, int64 NewSize, uint32 Alignment) = decltype(FMemory_Realloc)(__int64(GetModuleHandleW(0)) + 0x37CA240);

namespace Offsets
{
	constexpr int32 GObjects          = 0x09A6C6C8;
	constexpr int32 AppendString      = 0x038BF5F0;
	constexpr int32 GNames            = 0x00000000;
	constexpr int32 ProcessEvent      = 0x03ABFAA0;
...
public:
	...
	//uint8                                         Pad_0[0xC];                                       // 0x0008(0x0010)(Fixing Struct Size After Last Property [ Dumper-7 ])
...
static_assert(alignof(FUObjectItem) == 0x000008, "Wrong alinment on FUObjectItem");
...
//	{
//		Data = (T*)FMemory_Realloc(Data, (MaxElements = Num + NumElements) * sizeof(T), 0);
//	}
//
//	inline void Free()
//	{
...
public:
	...
	FORCEINLINE TArray(int32 Size)
		: NumElements(0), MaxElements(Size), Data(reinterpret_cast<InElementType*>(FMemory_Realloc(0, sizeof(InElementType) * Size, 0)))
	{
	}
	...
	FORCEINLINE InElementType& At(int i, size_t Size = sizeof(InElementType)) const { return *(InElementType*)(__int64(Data) + (static_cast<long long>(Size) * i)); }
	FORCEINLINE InElementType* AtPtr(int i, size_t Size = sizeof(InElementType)) const { return (InElementType*)(__int64(Data) + (static_cast<long long>(Size) * i)); }

	FORCEINLINE InElementType*& GetData() const { return Data; }
	FORCEINLINE InElementType*& GetData() { return Data; }
	...
	FORCEINLINE RangedForIteratorType      begin() { return Data; }
	FORCEINLINE RangedForConstIteratorType begin() const { return Data; }
	FORCEINLINE RangedForIteratorType      end() { return Data + NumElements; }
	FORCEINLINE RangedForConstIteratorType end() const { return Data + NumElements; }
	...
	FORCEINLINE void Reserve(const int Num, size_t Size = sizeof(InElementType))
	{
		Data = (InElementType*)FMemory_Realloc(Data, (MaxElements = Num + NumElements) * Size, 0);
	}

	FORCEINLINE InElementType& Add(const InElementType& InData, size_t Size = sizeof(InElementType))
	{
		Reserve(1, Size);

		memcpy_s((void*)(__int64(Data) + (NumElements * Size)), Size, (void*)&InData, Size);
		++NumElements;
	...
	FORCEINLINE bool Remove(int Index, size_t Size = sizeof(InElementType))
	{
		if (Index < NumElements)
		{
			if (Index != NumElements - 1)
			{
				memcpy_s(&At(Index, Size), Size, &At(NumElements - 1, Size), Size);
			}
...
static_assert(alignof(FName) == 0x000004, "Wrong alinment on FName");
...
static_assert(alignof(FTestStruct) == 0x000002, "Wrong alinment on FTestStruct");
...
static_assert(alignof(FTextData) == 0x000008, "Wrong alinment on FTextData");
...
static_assert(alignof(FText) == 0x000008, "Wrong alinment on FText");
...
static_assert(alignof(FWeakObjectPtr) == 0x000004, "Wrong alinment on FWeakObjectPtr");
...
static_assert(alignof(FUniqueObjectGuid) == 0x000004, "Wrong alinment on FUniqueObjectGuid");
...
static_assert(alignof(FScriptInterface) == 0x000008, "Wrong alinment on FScriptInterface");
...
static_assert(alignof(FFieldPath) == 0x000008, "Wrong alinment on FFieldPath");
...
static_assert(alignof(FFieldClass) == 0x000008, "Wrong alinment on FFieldClass");
...
static_assert(alignof(FFieldVariant) == 0x000008, "Wrong alinment on FFieldVariant");
...
static_assert(alignof(FField) == 0x000008, "Wrong alinment on FField");
...
static_assert(alignof(FProperty) == 0x000008, "Wrong alinment on FProperty");
...
static_assert(alignof(FByteProperty) == 0x000008, "Wrong alinment on FByteProperty");
...
static_assert(alignof(FBoolProperty) == 0x000008, "Wrong alinment on FBoolProperty");
...
static_assert(alignof(FObjectPropertyBase) == 0x000008, "Wrong alinment on FObjectPropertyBase");
...
static_assert(alignof(FClassProperty) == 0x000008, "Wrong alinment on FClassProperty");
...
static_assert(alignof(FStructProperty) == 0x000008, "Wrong alinment on FStructProperty");
...
static_assert(alignof(FArrayProperty) == 0x000008, "Wrong alinment on FArrayProperty");
...
static_assert(alignof(FDelegateProperty) == 0x000008, "Wrong alinment on FDelegateProperty");
...
static_assert(alignof(FMapProperty) == 0x000008, "Wrong alinment on FMapProperty");
...
static_assert(alignof(FSetProperty) == 0x000008, "Wrong alinment on FSetProperty");
...
static_assert(alignof(FEnumProperty) == 0x000008, "Wrong alinment on FEnumProperty");
...
static_assert(alignof(FFieldPathProperty) == 0x000008, "Wrong alinment on FFieldPathProperty");
...
static_assert(alignof(FOptionalProperty) == 0x000008, "Wrong alinment on FOptionalProperty");
