#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: MeltRuntime

#include "Basic.hpp"

#include "FortniteGame_classes.hpp"


namespace SDK
{

// Class MeltRuntime.FortAthenaMutator_Melt
// 0x0000 (0x03D0 - 0x03D0)
class AFortAthenaMutator_Melt final  : public AFortAthenaMutator_GameModeBase
{
public:
	static class UClass* StaticClass()
	{
		return StaticBPGeneratedClassImpl<"FortAthenaMutator_Melt">();
	}
	static class AFortAthenaMutator_Melt* GetDefaultObj()
	{
		return GetDefaultObjImpl<AFortAthenaMutator_Melt>();
	}
};
static_assert(alignof(AFortAthenaMutator_Melt) == 0x000008, "Wrong alinment on AFortAthenaMutator_Melt");
static_assert(sizeof(AFortAthenaMutator_Melt) == 0x0003D0, "Wrong size on AFortAthenaMutator_Melt");

}

