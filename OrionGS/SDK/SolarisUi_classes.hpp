#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: SolarisUi

#include "Basic.hpp"

#include "UMG_classes.hpp"


namespace SDK
{

// Class SolarisUi.SolUserWidget
// 0x0020 (0x0280 - 0x0260)
class USolUserWidget final  : public UUserWidget
{
public:
	uint8                                         Pad_4DD2[0x20];                                    // 0x0260(0x0020)(Fixing Struct Size After Last Property [ Dumper-7 ])

public:
	static class UClass* StaticClass()
	{
		return StaticBPGeneratedClassImpl<"SolUserWidget">();
	}
	static class USolUserWidget* GetDefaultObj()
	{
		return GetDefaultObjImpl<USolUserWidget>();
	}
};
static_assert(alignof(USolUserWidget) == 0x000008, "Wrong alinment on USolUserWidget");
static_assert(sizeof(USolUserWidget) == 0x000280, "Wrong size on USolUserWidget");

}

