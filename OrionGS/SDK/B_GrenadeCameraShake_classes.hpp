#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: B_GrenadeCameraShake

#include "Basic.hpp"

#include "GameplayCameras_classes.hpp"


namespace SDK
{

// BlueprintGeneratedClass B_GrenadeCameraShake.B_GrenadeCameraShake_C
// 0x0000 (0x01B0 - 0x01B0)
class UB_GrenadeCameraShake_C final  : public UMatineeCameraShake
{
public:
	static class UClass* StaticClass()
	{
		return StaticBPGeneratedClassImpl<"B_GrenadeCameraShake_C">();
	}
	static class UB_GrenadeCameraShake_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<UB_GrenadeCameraShake_C>();
	}
};
static_assert(alignof(UB_GrenadeCameraShake_C) == 0x000010, "Wrong alinment on UB_GrenadeCameraShake_C");
static_assert(sizeof(UB_GrenadeCameraShake_C) == 0x0001B0, "Wrong size on UB_GrenadeCameraShake_C");

}

